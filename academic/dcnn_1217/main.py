...
def main(_):
    ...
    if not os.path.exists("./result/runconfig"):
        os.makedirs("./result/runconfig")
    ...
    if FLAGS.train_black:
        train_data_black = Data(sampler_images=whole_black_path, standard_pic_dir=whole_standard_black_path,
                                ...
                                create_train_config=FLAGS.is_create_train_config, batch=FLAGS.batch_size, test_rate=0.05)
    ...
    with tf.Session(config=run_config) as sess:
        ...
        dcgan.train(config=FLAGS, data=train_data_white)
        '''luxb add begin'''
        #if FLAGS.train_black:
