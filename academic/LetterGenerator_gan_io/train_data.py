...
class Data:
    ...
    def get_batch_data(self, count):
        ...
        for line in lines:
            ...
            batch_labels.append(label_img)
        return np.array(batch_samples).astype(np.float32), np.array(batch_standards).astype(np.float32), np.array(batch_labels).astype(np.float32)
    ...
    def get_batch_data(self, count, sess):
        ...
        for line in lines:
            ...
            label_img = np.array(imread(os.path.join(self.sampler_images, line[2]), grayscale=False)) / 127.5 - 1.
            batch_labels.append(label_img)
            tensor_img = sess.run(get_4d_pic(sample_img, standard_img))
            batch_imgs.append(tensor_img)
        return np.array(batch_imgs).astype(np.float32), np.array(batch_labels).astype(np.float32)
