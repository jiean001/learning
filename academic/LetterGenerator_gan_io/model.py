...
class DCGAN(object):
    ...
    def train(self, config, train_data):
        ...
        sample_z = np.random.uniform(-1, 1, [config.batch_size, 28, 28, 4]) \
            .astype(np.float32)
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                if not train_data:
                    ...
                    first_imgs, standard_imgs, batch_images = train_data.get_batch_data(idx)
                    batch_z_ = self.sess.run(get_4d_pic(first_imgs, standard_imgs, axis=3))

                #batch_z = np.random.uniform(-1, 1, [config.batch_size, 28, 28, 4]) \
                #    .astype(np.float32)
                batch_z = batch_z_
                ...
                if np.mod(counter, 20) == 1:
                    try:
                        ...
                        save_images(sample_inputs, image_manifold_size(sample_inputs.shape[0]),
                                        './{}/origin_{:02d}_{:04d}.jpg'.format(config.sample_dir, epoch, idx))
    ...
    def generator(self, image, y=None, train=False, reuse=False):
        with tf.variable_scope("generator") as scope:
            ...
            if not self.y_dim:
                ...
                ''' luxb add convolution layer begin'''
                g_c_h0 = lrelu(conv2d(image, self.cnn_f_dim, name='g_h0_conv'))
                g_c_h1 = lrelu(self.g_c_bn1(conv2d(g_c_h0, self.cnn_f_dim * 2, name='g_h1_conv'), train=train))
                g_c_h2 = lrelu(self.g_c_bn2(conv2d(g_c_h1, self.cnn_f_dim * 4, name='g_h2_conv'), train=train))
                g_c_h3 = lrelu(self.g_c_bn3(conv2d(g_c_h2, self.cnn_f_dim * 8, name='g_h3_conv'), train=train))
                g_c_h4 = tf.reshape(g_c_h3, [self.batch_size, -1])
                '''luxb add convolution layer end'''
                ...
                self.z_, self.h0_w, self.h0_b = linear(
                    g_c_h4, self.gf_dim*8*s_h16*s_w16, 'g_h0_lin', with_w=True)
