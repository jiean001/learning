...
class LetterGenerator:
    ...
    def build_model(self):
        ...
        output_image_dim = [self.output_height, self.output_width, self.output_c_dim]
    ...
    def train(self, config, train_data):
        ...
        for epoch in range(config.epoch):
            batch_idxs = 1  # min(len(self.stain_data) / config.batch_size, config.train_size)
            for i in range(batch_idxs):
                batch_images, label_images = train_data.get_batch_data(i, self.sess)
                ...
                counter += 1
                print("Epoch: [%2d] [%4d/%4d] time: %4.4f loss:%.8f" %(epoch, i, batch_idxs, time.time() - start_time, loss))
