...
class ImageProcess:
    ...
    # format_size: the format size of output image eg: [28, 28]
    def __init__(self, gt_dir, s_dir, output_dir, output_black_config="format_0/config.txt", output_white_config="format_255/config.txt",
                 format_size=[28, 28]):
        """
        :param gt_dir:
        :param s_dir:
        :param output_dir:
        :param output_black_config:
        :param output_white_config:
        :param format_size:
        """
        # initial parameter
        self.gt_dir = gt_dir
        ...
        self.w_file = os.path.join(self.output_dir, output_white_config)
        self.b_file = os.path.join(self.output_dir, output_black_config)
        ...
        # global parameters
        self.style_count_white = 1
        self.style_count_black = 1
        self.letter_list_white = []
        ...
        self.bkgrd_color = [COLOR_BLACK, COLOR_WHITE]
    ...
    def add_style(self, is_new_file=False):
        if is_new_file:
            ...
            if len(self.letter_list_black) > 1:
                self.style_count_black += 1
            if len(self.letter_list_white) > 1:
                self.style_count_white += 1
        self.letter_list_black = []
        self.letter_list_white = []

    def add_style_log(self):
        letter_num_white = len(self.letter_list_white)
        ...
        if letter_num_white > 0:
            self.log("white style%d have %d letters" % (self.style_count_white, letter_num_white))
        if letter_num_black > 1:
            self.w_2_b("key : %sstyle%d" %(self.current_img, self.style_count_black))
        if letter_num_white > 1:
            self.w_2_w("key : %sstyle%d" %(self.current_img, self.style_count_white))
    ...
    def get_pic_name(self, current_dir_b, current_dir_w, is_save_b=True, is_save_w=True):
        ...
        if is_save_w:
            self.letter_list_white.append(self.letter)
            letter_num_w = int(self.letter_list_white.count(self.letter))
            letter_name_w = ("%s%sstyle%d_%c_%d.jpg" % (current_dir_w, self.current_img, self.style_count_white, self.letter, letter_num_w))
    ...
    def get_origin_letter_255(self, filter_0, letter_0):
        filter_255 = (filter_0 + COLOR_WHITE) % (COLOR_WHITE + 1)
    ...
    def is_save_color(self, letter_0, filter_0):
        ...
        if abs(COLOR_BLACK - mean_color) < MIN_DELTA_COLOR_BLACK:
            ...
            save_0 = False
        if abs(COLOR_WHITE - mean_color) < MIN_DELTA_COLOR_WHITE:
            self.log(
                "NO SAVE: %s, The color(%f, %f) of the gap is too small." % (self.line, mean_color, COLOR_WHITE))
            save_255 = False
    ...
    def get_letter(self, img, gt_img):
        ...
        if origin_name_255 != "":
            ...
            elif len(self.letter_list_white) == 2:
        ...
        for img_dir in self.imgDirs:
            ...
            for idx in bf:
                ...
                if not len(idx):
                    if len(self.letter_list_white) == 1:
                        self.log(str(idx))
                        self.log("error! not enough white letters, so we do not save this letter")
                        print("error! not enough white letters, so we do not save this letter")
                    if len(self.letter_list_black) == 1:
                        self.log(str(idx))
